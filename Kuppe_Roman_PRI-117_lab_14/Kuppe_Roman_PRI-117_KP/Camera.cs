...
namespace Kuppe_Roman_PRI_117_lab_14 {
    class Camera {
        ...
        public void PositionCamera(float pos_x, float pos_y, float pos_z, float view_x, float view_y, float view_z, float up_x, float up_y, float up_z) {
            // Позиция камеры
            mPos.x = pos_x;  
            mPos.y = pos_y;  
            mPos.z = pos_z;
